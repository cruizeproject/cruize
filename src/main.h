...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.09 * COIN;	// 9% Interest 
static const int64_t COIN_YEAR_REWARD = 9 * CENT; //(output to console will be updated)
...
public:
    ...
    {
        // cruize: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // cruize: get transaction coin age
...
public:
    ...
    // cruize: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // cruize: two types of block: proof-of-work or proof-of-stake
    ...
    // cruize: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // cruize: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // cruize: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // cruize: block index flags
    enum  
    {
